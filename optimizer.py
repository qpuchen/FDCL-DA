...
def build_optimizer(config, model, simmim=False, is_pretrain=False):
    ...
    if simmim:
        ...
        parameters = set_weight_decay(model, skip, skip_keywords, config=config)
...
def set_weight_decay(model, skip_list=(), skip_keywords=(),config=None):
    has_decay = []
    no_decay = []
    head_has_decay = model.head.weight
    head_no_decay = model.head.bias

    for name, param in model.named_parameters():
        if not param.requires_grad or name.startswith('head'):
    ...
    return [{'params': has_decay, 'lr': config.TRAIN.BASE_LR},
            {'params': no_decay, 'weight_decay': 0., 'lr': config.TRAIN.BASE_LR},
            {'params': head_no_decay, 'weight_decay': 0., 'lr': config.TRAIN.BASE_LR * config.TRAIN.HEAD_MUL},
            {'params': head_has_decay, 'lr': config.TRAIN.BASE_LR * config.TRAIN.HEAD_MUL}]
