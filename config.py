...
_C.TRAIN.MIN_LR = _C.TRAIN.BASE_LR/10**2
# Clip gradient norm
...
def update_config(config, args):
    ...
    if _check_args('use_selection'):
        config.TRAIN.use_selection = args.use_selection

    if _check_args('num_part'):
        config.TRAIN.NUM_PART = args.num_part

    if _check_args('swap_w'):
        config.TRAIN.SWAP_W = args.swap_w
