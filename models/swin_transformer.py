...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        x_, feat_dict = self.forward_features(x)
        x = self.head(x_)

        logits_dict ={}
        if self.use_selection:
            self.selector = WeaklySelector(feat_dict, self.num_classes, self.num_selects)
            logits_dict = self.selector(feat_dict, logits_dict)

        return x, x_, logits_dict
        # return x
...
class WeaklySelector(nn.Module):
    ...
    def __init__(self, inputs: dict, num_classes: int, num_select: dict):
        ...
        for name in inputs:
            ...
            if len(fs_size) == 3:
                ...
                in_size = fs_size[1]
            m = nn.Linear(in_size, num_classes).cuda()
    ...
    def forward(self, x, logits=None):
        ...
        for name in x:
            ...
            logits[name] = getattr(self, "classifier_l_" + name)(x[name]).float()
