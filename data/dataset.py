...
class ImageDataset():
    ...
    def __getitem__(self, index):
        ...
        target = self.data_infos[index]["label"]
        ...
        # only image in base, out + mask/cov id in sim mm
        return out, target
...
class leaf_hair():
    ...
    def __getitem__(self, index):
        ...
        img, target, imgname = imageio.imread(img_path), self.label_remap[self.anno[index]['label']], self.anno[index][
            'id']
        if len(img.shape) == 2:
            img = np.stack([img] * 3, 2)
        out = Image.fromarray(img, mode='RGB')
...
class CarsDataset(Dataset):
    ...
    def __init__(self, root, is_train=True, cleaned=None, transform=None):
        """
        Args:
        ...
        if is_train:
            data_dir = os.path.join(root, 'cars_train')
        else:
            data_dir = os.path.join(root, 'cars_test')
