...
def build_loader(config, logger, is_train=True):
    ...
    if config.DATA.DATASET == 'imagenet':
        ...
        dataset = NABirds(root=data_root, train=is_train, transform=transform)
    elif config.DATA.DATASET == 'Aircraft':
        dataset = FGVC_aircraft(root=data_root, is_train=is_train, transform=transform)
    # elif config.DATA.DATASET == 'stanford_car':
    #     dataset = STANFORD_CAR(root=data_root, is_train=is_train, transform=transform)
    elif config.DATA.DATASET == 'car':
        dataset = CarsDataset(root=data_root, is_train=is_train, transform=transform)
    elif config.DATA.DATASET == 'rank_grass':
        dataset = ImageDataset(root=data_root, is_train=is_train, transform=transform)
    ...
    # elif config.DATA.DATASET == "stanford_car":
    #     num_classes = 196
    elif config.DATA.DATASET == 'plant_pathology':
        num_classes = 4
    elif config.DATA.DATASET == 'bird_species':
        num_classes = 25
    elif config.DATA.DATASET == 'leaf_disease':
        num_classes = 4
    elif config.DATA.DATASET == 'leaf_disease_new':
        num_classes = 3
    elif config.DATA.DATASET == 'rank_grass':
        num_classes = 8
    else:
